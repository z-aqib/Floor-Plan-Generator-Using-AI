...
//        System.out.println("length by FloorPlanWindow -> " + length);
        scale = (Math.min(screenPanel[2], screenPanel[3]) - 50) / height;
//        System.out.println("scale by FloorPlanWindow -> " + scale);
        int[][] roomSize = new int[floorPlan.length][2];
...
//                System.out.println("rooms[j] in FloorPlanWindow -> " + rooms[j]);
                if (rooms[j].contains(roomName) == true) {
                    ...
                    roomSize[i] = room.findSize(Integer.parseInt(area[1].substring(1)));
//                    System.out.println("asigned roomlength in FloorPlanWindow -> " + roomSize[i][0] + " , " + roomSize[i][1]);
                    break;
                }
            }
        }
...
//        System.out.println("length -> " + length);
        topLeft = new Point(screenPanel[0] + 50, screenPanel[1] + 25);
        ...
        for (int i = 0; i < roomButtons.length; i++) {
            int roomLength = (int) (roomSize[i][0] * scale);
            int roomWidth = (int) (roomSize[i][1] * scale);
            int x = 0;
            int y = rowComplete + 1;
            boolean fitted;
            do {
