...
//            background_panel[2], background_panel[3] - limit_to_decrease});
        ...
        // menu buttons
        this.menu_strings = new String[]{"Welcome", "", "Design"};
...
//            }
        ...
        if (y < limit_to_decrease) {
            for (int i = 0; i < menu_buttons.length; i++) {
                if (menu_buttons[i].getName().equals("") == false && menu_buttons[i].ifSelected(x, y) == true) {
                    window_number = i + 1;
                }
            }
        }
...
//            }
    ...
    public void mouseMoved(int x, int y) {
        for (int i = 0; i < menu_buttons.length; i++) {
            if (menu_buttons[i].getName().equals("") == false) {
